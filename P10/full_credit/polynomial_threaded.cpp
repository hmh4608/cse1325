...
void Polynomial::solve(double min, double max, int nthreads, double slices, double precision) {
    ...
    //calculate the difference between max and min for each range
    ...
    { 
	threads[i] = std::thread{&Polynomial::solve_recursive, this, min, max, i, slices, precision, 1};
	min += delta;
	max += delta;
    }

    //let's bundle them up again
    for(int i=0; i<nthreads; ++i)
    {
	threads[i].join();
    }
